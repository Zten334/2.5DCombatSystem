...
如果你想要导出一个enum，那么请注意：1，当你更新enum的时候，导出栏的enum并不会随之更新，你需要手动刷新代码，这目前是个bug。2.如果你在导出的时候，又用enum创建了一个字典或者列表，那么恭喜你，如果你不在enum中手动给枚举指定一个值，那么你新增之后，值会自动增加，而导致你原来的enum和修改后的enum不一致！而且这不会报错，只会产生一个幽灵枚举。听不懂也没事，问AI，让AI告诉你什么意思（